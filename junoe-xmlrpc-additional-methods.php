...
function junoe_confluence_emoticon($atts) {
    ...
    $ret = $name;
    if (isset($accespts[$name])){
        $ret = sprintf('<img src="%s/%s" title="%s" class="confluence-emoticon" width="16" height="16"/>',
                       $icon_path, $accespts[$name], $name);
    }
    
    return $ret;
}
add_shortcode('confluence-emoticon', 'junoe_confluence_emoticon');
